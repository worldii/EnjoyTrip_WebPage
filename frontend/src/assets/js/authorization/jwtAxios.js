...
async function regenerateToken(){
    jwtAxios.defaults.headers.common[REFRESH_TOKEN_HEADER]=sessionStorage.getItem(REFRESH_TOKEN_HEADER);

    return await jwtAxios
        .post(REFRESH_URI, {userId: JSON.parse(sessionStorage.getItem("vuex")).userStore.userId})
        .then(({data,status}) => {
